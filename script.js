...
const sortedByBirthYear = inventors.sort((low, high) => {
...
const inventorNamedAda = inventors.find((inventors) => inventors.first === 'Ada')
...
const totalYearsLived = inventors.reduce((acc, age) => {
...
const travelMethodCounts = travelMethods.reduce((acc, travelMethod) => {
    if (acc[travelMethod]) {
        acc[travelMethod] += 1;
    } else {
        acc[travelMethod] = 1;
